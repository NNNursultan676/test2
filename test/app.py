...
def send_telegram_notification(user_id, message):
    """Send notification to user via Telegram bot with auto-delete"""
    try:
            ...
            if message_id:
                ...
                def delete_message_after_delay():
                    ...
                    try:
                        ...
                        logging.info(f"Auto-deleted message {message_id} for user {user_id}")
                    except Exception as e:
                        logging.warning(f"Failed to auto-delete message {message_id}: {e}")
                
                # Start deletion thread
                threading.Thread(target=delete_message_after_delay, daemon=True).start()
...
def send_notification_to_all_users(message):
    ...
    for telegram_id in users:
        if send_telegram_notification(int(telegram_id), message):
            success_count += 1
...
def create_notification():
    ...
    new_notification = {
        ...
        'repeat_count': 3,  # Всегда 3 отправки
        'repeat_interval_minutes': 10,  # Интервал 10 минут
    ...
    if save_notifications(notifications):
        flash(f'Уведомление создано! Будет отправляться в {notification_time} по дням: {", ".join(days_of_week)} (3 раза с интервалом 10 минут)', 'success')
    else:
