...
def create_notification():
    ...
    if save_notifications(notifications):
        # Send the actual notification message to Telegram group immediately
        try:
            import sys
            import os
            sys.path.append(os.path.dirname(__file__))
            from bot import send_notification_to_group_sync

            # Send the actual notification message (not the creation message)
            success = send_notification_to_group_sync(message)
            if success:
                flash('Уведомление создано и отправлено в группу!', 'success')
            else:
            ...
            flash('Уведомление создано, но произошла ошибка при отправке в группу', 'warning')
    else:
...
def create_recurring_notification():
    ...
    if save_recurring_notifications(recurring_notifications):
        ...
        try:
            ...
            success = send_notification_to_group_sync(message)
            if success:
                flash(f'Повторяющееся уведомление создано и отправлено в группу!', 'success')
            else:
                flash(f'Повторяющееся уведомление создано, но не удалось отправить в группу', 'warning')
        except Exception as e:
            logger.error(f"Error sending recurring notification to Telegram: {e}")
            flash(f'Повторяющееся уведомление создано, но произошла ошибка при отправке в группу', 'warning')
    else:
