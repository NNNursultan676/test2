...
from config import BOT_TOKEN, GROUP_ID, USERS_JSON_PATH, BOOKINGS_JSON_PATH
...
def delete_booking(booking_id):
    ...
    for i, booking in enumerate(bookings):
        # Allow deletion if user owns booking or is admin
        if (booking['id'] == booking_id and 
            (str(booking.get('telegram_id')) == str(telegram_id) or admin_level > 0)):
            booking_to_delete = i
            break
