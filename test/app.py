...
def load_notifications():
    ...
    try:
        notifications_path = os.path.join(os.path.dirname(BOOKINGS_JSON_PATH), 'notifications.json')
...
def load_recurring_notifications():
    ...
    try:
        recurring_notifications_path = os.path.join(os.path.dirname(BOOKINGS_JSON_PATH), 'recurring_notifications.json')
...
def clear_all_system_data():
    """Clear all bookings and notifications (only for level 3 admins)"""
    try:
        # Clear bookings
        if save_bookings([]):
...
def send_recurring_notification_to_group(message):
    ...
    try:
        ...
        logging.info(f"Sent recurring notification to group: {result.get('ok', False)} - Response: {result}")
        return result.get('ok', False)
    except Exception as e:
        logging.error(f"Error sending recurring notification to group: {e}")
        return False
...
def create_notification():
    ...
    if save_notifications(notifications):
        ...
        test_message = f"📢 Новое уведомление создано:\n\n{message}\n\n⏰ Будет отправляться: {', '.join(days_of_week)} в {notification_time}\n🔄 Повторений: {repeat_count}\n📅 Недель: {weeks_count}"
        send_success = send_group_notification(test_message, thread_id if thread_id else THREAD_ID)
        
        if send_success:
            flash('Уведомление создано и отправлено в группу успешно', 'success')
        else:
            flash('Уведомление создано, но не удалось отправить в группу', 'warning')
    else:
...
def create_recurring_notification():
    ...
    if save_recurring_notifications(recurring_notifications):
        # Send test notification to group
        test_message = f"🔄 Новое повторяющееся уведомление создано:\n\n{message}\n\n⏰ Будет отправляться: {', '.join(days_of_week)} в {notification_time}\n🔄 Повторений: {repeat_count} раз\n📅 Недель подряд: {weeks_count}\n⏱️ Интервал: каждые {repeat_interval} часов"
        send_success = send_recurring_notification_to_group(test_message)
        
        if send_success:
            flash('Повторяющееся уведомление создано и отправлено в группу успешно', 'success')
        else:
            flash('Повторяющееся уведомление создано, но не удалось отправить в группу', 'warning')
    else:
