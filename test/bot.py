...
async def send_notification_to_all_users(message, delete_after_minutes=5):
    """Send notification to all users with auto-delete"""
    try:
        # Load users from app data
        ...
        success_count = 0
        for telegram_id in users_data.keys():
            try:
                url = f"https://api.telegram.org/bot{BOT_TOKEN}/sendMessage"
                data = {
                ...
                async with aiohttp.ClientSession() as session:
                    async with session.post(url, json=data) as response:
                        ...
                        if result.get('ok'):
                            ...
                            # Schedule deletion for first 2 messages (not the last one)
                            if delete_after_minutes > 0:
                                asyncio.create_task(auto_delete_message_delayed(
                                    int(telegram_id), message_id, delete_after_minutes * 60
                                ))
                            
                            logger.info(f"✅ Sent notification to user {telegram_id}")
                        else:
                            logger.warning(f"❌ Failed to send to user {telegram_id}: {result.get('description')}")
                            
            except Exception as e:
                logger.error(f"Error sending to user {telegram_id}: {e}")
                continue
                
        logger.info(f"📤 Notification sent to {success_count}/{len(users_data)} users")
        return success_count
        
    except Exception as e:
        logger.error(f"Error in send_notification_to_all_users: {e}")
        return 0

def load_users_data():
    """Load users data from Flask app's JSON file"""
    try:
        import os
        ...
        logger.warning("Users file not found")
        return {}
    except Exception as e:
        logger.error(f"Error loading users: {e}")
        return {}

async def auto_delete_message_delayed(chat_id, message_id, delay_seconds):
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data) as response:
                result = await response.json()
                if result.get('ok'):
                    logger.info(f"🗑️ Auto-deleted message {message_id} for user {chat_id}")
                else:
                    logger.warning(f"Failed to delete message {message_id}: {result.get('description')}")
                    
    except Exception as e:
        logger.warning(f"Error auto-deleting message {message_id}: {e}")

async def process_recurring_notifications(context: ContextTypes.DEFAULT_TYPE):
    """Process and send notifications - 3 messages with 10 minute intervals"""
    try:
        ...
        logger.info(f"🔔 Processing notifications at {current_time.strftime('%H:%M')} on {current_day}")
        ...
        for notification in notifications:
            ...
            try:
                ...
                # Check if it's the right time (with 1 minute tolerance)
                if abs(current_minute - notification_minute) > 1:
                    continue

                sent_count = notification.get('sent_count', 0)
                
                # First message - send at scheduled time
                if sent_count == 0:
                    message = f"📢 {notification['message']}"
                    success_count = await send_notification_to_all_users(message, delete_after_minutes=5)
                    if success_count > 0:
                        notification['sent_count'] = 1
                        notification['last_sent'] = now.isoformat()
                        updated = True
                        logger.info(f"✅ Sent first notification (1/3): {notification['message'][:50]}...")

                # Second and third messages - check 10 minute intervals
                elif sent_count < 3:
                    ...
                    if last_sent:
                        ...
                        if time_since_last >= 600:  # 10 minutes = 600 seconds
                            # Determine if this is the last message (no auto-delete)
                            is_last_message = (sent_count == 2)
                            delete_minutes = 0 if is_last_message else 5
                            
                            message = f"🔄 {notification['message']}"
                            success_count = await send_notification_to_all_users(message, delete_after_minutes=delete_minutes)
                            if success_count > 0:
                                notification['sent_count'] = sent_count + 1
                                notification['last_sent'] = now.isoformat()
                                
                                # Mark as inactive if this was the third message
                                if notification['sent_count'] >= 3:
                                    notification['is_active'] = False
                                    logger.info(f"🏁 Notification completed all 3 sends: {notification['message'][:50]}...")
                                
                                updated = True
                                logger.info(f"✅ Sent notification ({notification['sent_count']}/3): {notification['message'][:50]}...")
        ...
        if updated:
            if save_notifications(notifications):
                logger.info("📁 Notifications updated and saved")
            else:
