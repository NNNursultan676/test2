...
async def send_recurring_notification_to_group(message):
    ...
    try:
        ...
        if result.get('ok'):
            logger.info(f"Successfully sent recurring notification to group")
        else:
            logger.error(f"Failed to send notification: {result.get('description', 'Unknown error')}")
...
async def process_recurring_notifications(context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logger.info(f"Processing notifications at {current_time} on {current_day}")
        ...
        for notification in recurring_notifications:
            ...
            try:
                ...
                # Check if it's the right time (exact minute match)
                if abs(current_minute - notification_minute) > 1:  # Allow 1 minute tolerance
                    continue
                ...
                if last_sent:
                    ...
                    interval_seconds = notification.get('repeat_interval_seconds', 86400)
                    
                    time_since_last = (now - last_sent_time).total_seconds()
                    
                    # Send if interval has passed (with 1 minute tolerance)
                    if time_since_last >= (interval_seconds - 60):
                        success = await send_recurring_notification_to_group(notification['message'])
                        if success:
                            ...
                            if notification['sent_count'] >= repeat_count:
                                ...
                                logger.info(f"Recurring notification {notification.get('id', 'unknown')} completed")

            except Exception as e:
                logger.error(f"Error processing recurring notification {notification.get('id', 'unknown')}: {e}")
                continue

        # Process regular notifications with same logic
