    ...
    try:
        ...
        if admin_level >= 2:
            keyboard.extend([
                ...
                [InlineKeyboardButton("🔔 Удалить все уведомления", callback_data="clear_all_notifications")]
            ])
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    elif query.data == "clear_all_notifications":
        if admin_level >= 2:
            # Clear all notifications and recurring notifications
            try:
                import json
                import os
...
async def send_private_message(user_id, message):
    """Send notification to user's private messages"""
    try:
        import aiohttp
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(url, json=data) as response:
                ...
                if result.get('ok'):
                    logger.info(f"✅ Successfully sent private message to user {user_id}")
                    return True
                else:
                    error_desc = result.get('description', 'Unknown error')
                    logger.error(f"❌ Failed to send private message to user {user_id}: {error_desc}")
                    return False
...
async def process_notifications(context: ContextTypes.DEFAULT_TYPE):
    """Process and send notifications to users' private messages"""
    try:
        ...
        for user_key, user_data in users.items():
            # Extract telegram_id from user key or data
            try:
                # Try to get telegram_id from key format: "name_company_timestamp.telegram_id"
                if '_' in user_key and '.' in user_key:
                    parts = user_key.split('.')
                    if len(parts) == 2:
                        telegram_id = int(parts[1])
                        registered_users.append(telegram_id)
                        continue
                
                # Try to get from user data if exists
        ...
        logger.info(f"📊 Found {len(registered_users)} registered users for notifications")
        ...
        for notification in recurring_notifications:
            ...
            try:
                ...
                repeat_count = min(notification.get('repeat_count', 1), 3)  # Maximum 3 repeats

                # For first send - send at scheduled time
                if sent_count == 0:
                    message = f"🔔 <b>Уведомление</b>\n\n{notification['message']}"
                    logger.info(f"📤 Sending first recurring notification: {notification['message'][:50]}...")
                    ...
                    if success_count > 0:
                        ...
                        logger.info(f"✅ Sent first recurring notification {notification.get('id', 'unknown')}: 1/{repeat_count} to {success_count} users")
                    else:
                        logger.error(f"❌ Failed to send first recurring notification {notification.get('id', 'unknown')} to any user")
                    continue
                ...
                if last_sent:
                    ...
                    if time_since_last >= (interval_seconds - 120):
                        message = f"🔄 <b>Повторное уведомление</b>\n\n{notification['message']}"
                        ...
                        if success_count > 0:
                            ...
                            notification['last_sent'] = now.isoformat()
                            updated_recurring = True
                            logger.info(f"✅ Sent recurring notification {notification.get('id', 'unknown')}: {sent_count + 1}/{repeat_count} to {success_count} users")
                            ...
                            logger.error(f"❌ Failed to send recurring notification {notification.get('id', 'unknown')} to any user")
            ...
            try:
                ...
                if sent_count == 0:
                    ...
                    if success_count > 0:
                        ...
                        logger.info(f"✅ Sent first regular notification {notification.get('id', 'unknown')}: 1/{repeat_count} to {success_count} users")
                    continue
                ...
                if last_sent:
                    ...
                    if time_since_last >= (interval_seconds - 120):
                        message = f"📢 <b>Повторное объявление</b>\n\n{notification['message']}"
                        ...
                        if success_count > 0:
                            ...
                            notification['last_sent'] = now.isoformat()
                            updated_regular = True
                            logger.info(f"✅ Sent regular notification {notification.get('id', 'unknown')}: {sent_count + 1}/{repeat_count} to {success_count} users")
...
def main() -> None:
    ...
    # Add job queue for notifications (check every minute)
    job_queue = application.job_queue
    if job_queue:
        job_queue.run_repeating(process_notifications, interval=60, first=10)
        logger.info("Job queue for notifications started - sending to private messages")

    # Log bot startup
    logger.info("Starting Sapa Room Booking Bot with private message notifications...")
