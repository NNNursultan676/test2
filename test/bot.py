...
async def send_recurring_notification_to_group(message):
    ...
    try:
        ...
        logger.info(f"Sent recurring notification to group: {result.get('ok', False)}")
    ...
    try:
        ...
        for notification in recurring_notifications:
            ...
            if success:
                ...
                updated = True
                logger.info(f"Sent recurring notification {notification['id']}: {sent_count + 1}/{repeat_count}")
                
                # Deactivate if all repeats are done
                if notification['sent_count'] >= repeat_count:
                    notification['is_active'] = False
                    logger.info(f"Recurring notification {notification['id']} completed all repeats")
